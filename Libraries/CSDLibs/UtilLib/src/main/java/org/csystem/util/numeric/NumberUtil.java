...
public final class NumberUtil {
    ...
    public static int indexOfPrime(BigInteger val)
    {
        var i = 1;
    ...
    public static BigInteger sumFactors(BigInteger val)
    {
        ...
        for (var i = BigInteger.TWO; i.compareTo(sqrtVal) <= 0; i = i.add(BigInteger.ONE))
            if (val.remainder(i).equals(BigInteger.ZERO)){
                var divide = val.divide(i);
                result = result.add(i.equals(divide) ? i : (i.add(divide)));
            }
        return result;
    }
